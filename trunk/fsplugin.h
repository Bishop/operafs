...
// Function prototypes
TC_API int FsInit(int PluginNr, tProgressProc pProgressProc,
					tLogProc pLogProc, tRequestProc pRequestProc);
TC_API HANDLE FsFindFirst(char* Path,WIN32_FIND_DATA *FindData);
TC_API BOOL FsFindNext(HANDLE Hdl,WIN32_FIND_DATA *FindData);
TC_API int FsFindClose(HANDLE Hdl);

BOOL __stdcall FsMkDir(char* Path);
TC_API int FsExecuteFile(HWND MainWin,char* RemoteName,char* Verb);
int __stdcall FsRenMovFile(char* OldName,char* NewName,BOOL Move,
                           BOOL OverWrite,RemoteInfoStruct* ri);
TC_API int FsGetFile(char* RemoteName, char* LocalName, int CopyFlags, RemoteInfoStruct* ri);
TC_API int FsPutFile(char* LocalName, char* RemoteName, int CopyFlags);
TC_API BOOL FsDeleteFile(char* RemoteName);
TC_API BOOL FsRemoveDir(char* RemoteName);
...
TC_API void FsGetDefRootName(char* DefRootName, int maxlen);
TC_API int FsExtractCustomIcon(char* RemoteName,int ExtractFlags,HICON* TheIcon);

TC_API void FsSetDefaultParams(FsDefaultParamStruct* dps);
int __stdcall FsGetPreviewBitmap(char* RemoteName,int width,int height,HBITMAP* ReturnedBitmap);
TC_API BOOL FsLinksToLocalFiles(void);
TC_API BOOL FsGetLocalName(char* RemoteName,int maxlen);
...
TC_API int FsContentGetSupportedField(int FieldIndex, char* FieldName, char* Units, int maxlen);
TC_API int FsContentGetValue(char* FileName,int FieldIndex,int UnitIndex,void* FieldValue,int maxlen,int flags);

void __stdcall FsContentStopGetValue(char* FileName);
int __stdcall FsContentGetDefaultSortOrder(int FieldIndex);
TC_API void FsContentPluginUnloading(void);
TC_API int FsContentGetSupportedFieldFlags(int FieldIndex);

int __stdcall FsContentSetValue(char* FileName,int FieldIndex,int UnitIndex,int FieldType,void* FieldValue,int flags);
TC_API BOOL FsContentGetDefaultView(char* ViewContents,char* ViewHeaders,char* ViewWidths,char* ViewOptions,int maxlen);